...
| `yamlComment`             | `string`  | no       | Comment written at the top of every YAML document VAYS generates itself, i.e. the document seeded when **creating** an entity (right after the `---` marker, before the data). Each line of the text becomes a `# ` comment line; an empty string omits the comment altogether. Defaults to `Automatically generated by VAYS`. |
...
```json
{
  ...
  "yamlComment": "Managed via the Example Corp self-service portal - edit there, not by hand",
  "oidcConf": {
